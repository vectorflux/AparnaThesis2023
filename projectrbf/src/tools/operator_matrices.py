...
def differentiation (inv_A,xyz_r,Xj,normalization_factor):
    ...
    for i in range(n):
        ...
        mult_constant = wendland1_prime(r_x) / ( r_x * normalization_factor * normalization_factor )
...
def gradient(c,xyz_r,Xj,normalization_factor):
    ...
    for i in range(n):
        r_x = np.linalg.norm(xyz_r[i,:]-Xj)/normalization_factor #scaling to match wendland functions
        mult_constant = wendland_3_1_prime(r_x) / ( r_x * normalization_factor * normalization_factor ) # derivative $\phi'$
